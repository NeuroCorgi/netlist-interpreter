  ...
  where
    ...
    go oldMem@Memory{mUpdated=updates} = newMem
      where
        ...
        -- Node state update functions can run in parallel as a possible optimization
        newMem = melt oldMem $ map (\(Node (f, ins, outs)) -> (, outs) $ f $ freeze oldMem (ins ++ outs)) influencedNodes

        -- Freeze memory inputs for cells
        freeze :: Memory [] -> [Int] -> Memory IntMap
        freeze Memory{..} freezeInds = Memory
          -- Memory consists only of inputs and outputs of a node
        ...
        -- A creative name instead of a functional one, unfreeze and stage updates recieved from the cells
        ...
        melt (Memory{mMemory=origMem}) cellMem = Memory
          ...
          -- Init is needed to cut the previous, now processed, update
          , mUpdated = concatMap (init' . mUpdated . fst) cellMem
          , mSubMem = mempty
          , mSubDes = mempty }
          where
            -- Sort is needed because of how `patch` works
